...
public class MessageController {
    ...
    public ResponseEntity postMessage(@Valid @RequestBody MessagePostDto messagePostDto) {
        Message message = messageService.createMessage(mapper.messagePostDtoToMessage(messagePostDto));

//        return ResponseEntity.ok(mapper.messageToMessageResponseDto(message));
        return new ResponseEntity<>(mapper.messageToMessageResponseDto(message), HttpStatus.OK);
    }
